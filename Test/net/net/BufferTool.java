...
public class BufferTool {
	...
	public BufferTool(InputStream inputStream,int bufferLength){
		this.inputStream=inputStream;
		this.maxBufferLength=bufferLength;
		this.bufferData=new byte[bufferLength];
	}

	public BufferTool(InputStream inputStream,int realDataLength,int maxBufferLength){
		this.inputStream = inputStream;
	...
	public BufferTool(InputStream inputStream){
...
//			e.printStackTrace();
	...
	private void readFromInputStream(int dataLength) throws IOException{
		...
		// yuyue 解决可能读取错位问题 start
//		int needLength=maxBufferLength-lastLength;//还需要读取的字节长度
		int needLength=maxBufferLength-realBufferLength;//还需要读取的字节长度
		// yuyue 解决可能读取错位问题 end
...
//		}
		...
		if (realReadLength == -1 && lastLength < dataLength) {
			String exceptionMessage = "realReadLength = -1 , realBufferLength = " + lastLength + ", dataLength = " + dataLength;
