...
public class UserJDBC extends JDBCTemplate {
	...
	protected void setQuerySqlValue(PreparedStatement pstmt, Object qm)
			throws Exception {
		UserQueryModel uqm = (UserQueryModel) qm;
		int count = 1;
		if (uqm.getUuid() != null && uqm.getUuid().trim().length() > 0) {
	...
	private void setCreateValue(PreparedStatement pstmt, UserModel um)
			throws Exception {
	...
	private void setUpdateValue(PreparedStatement pstmt, UserModel um)
			throws Exception {
	...
	private void setDeleteValue(PreparedStatement pstmt, UserModel um)
			throws Exception {
		pstmt.setString(1, um.getUuid());
	}
}
