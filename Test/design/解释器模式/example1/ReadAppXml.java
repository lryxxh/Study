...
public class ReadAppXml {
	...
	public void read(String filePathName) throws Exception {
		...
		// ((Element)jdbc.item(0)).getElementsByTagName("driver-class");
		// String driverClass =
		// driverClassNode.item(0).getFirstChild().getNodeValue();
		// System.out.println("driverClass=="+driverClass);
		// //同理获取url、user、password等值
		// NodeList urlNode =
		// ((Element)jdbc.item(0)).getElementsByTagName("url");
		// String url = urlNode.item(0).getFirstChild().getNodeValue();
		// System.out.println("url=="+url);
		//
		// NodeList userNode =
		// ((Element)jdbc.item(0)).getElementsByTagName("user");
		// String user = userNode.item(0).getFirstChild().getNodeValue();
		// System.out.println("user=="+user);
		//
		// NodeList passwordNode =
		// ((Element)jdbc.item(0)).getElementsByTagName("password");
		// String password =
		// passwordNode.item(0).getFirstChild().getNodeValue();
		...
		// applicationXmlNode.item(0).getFirstChild().getNodeValue();
		...
		for (int i = 0; i < appXmlNode.getLength(); i++) {
			String applicationXml = appXmlNode.item(i).getFirstChild()
					.getNodeValue();
			System.out.println("applicationXml==" + applicationXml);
		}
	}
