 ...
 * {@link Matcher2} object that can match arbitrary
 ...
 * in the Matcher2, so many Matcher2s can share the same pattern.
 * 
 * <p>
 ...
 * Matcher2 m = p.{@link #Matcher2 Matcher2}("aaaaab");
 * boolean b = m.{@link Matcher2#matches matches}();
 * </pre>
 * 
 * </blockquote>
 * 
 ...
 * concurrent threads. Instances of the {@link Matcher2} class are not safe for
 * such use.
 * 
 * 
 * <a name="sum">
 ...
 * </tr>
 * <tr align="left">
 * <th colspan="2" id="bounds">Boundary Matcher2s</th>
 * </tr>
 * 
 * <tr>
 ...
 * may also be retrieved from the Matcher2 once the match operation is complete.
 * 
 * <p>
 ...
 * {@link Matcher2} class: Repeated invocations of the {@link Matcher2#find
 * find} method will resume where the last match left off, unless the Matcher2
 * is reset.
 * </p>
 * </li>
 * 
...
public final class Pattern2 implements java.io.Serializable {
	...
	/**
	 * The number of capturing groups in this Pattern. Used by Matcher2s to
	 * allocate storage needed to perform a match.
	 */
	transient int capturingGroupCount;

	/**
	 * The local variable count used by parsing tree. Used by Matcher2s to
	 * allocate storage needed to perform a match.
	 */
	...
	/**
	 * Creates a Matcher2 that will match the given input against this pattern.
	 * </p>
	 * 
	 * @param input
	 *            The character sequence to be matched
	 * 
	 * @return A new Matcher2 for this pattern
	 */
	public Matcher2 matcher(CharSequence input) {
		...
		Matcher2 m = new Matcher2(this, input);
		return m;
	}
	 ...
	 * 
	 * <pre>
	 * Pattern.compile(regex).Matcher2(input).matches()
	 * </pre>
	 * 
	 * </blockquote>
	 * 
	...
	public static boolean matches(String regex, CharSequence input) {
		Pattern2 p = Pattern2.compile(regex);
		Matcher2 m = p.matcher(input);
		return m.matches();
	}
	 ...
	 */
	private Pattern2(String p, int f) {
		pattern = p;
		flags = f;
	...
	/**
	 * Returns a suitably optimized, single character Matcher2.
	 */
	...
	/**
	 * Utility method for creating a string slice Matcher2.
	 */
	...
	static class Node extends Object {
		...
		Node() {
			next = Pattern2.accept;
		}

		/**
		 * This method implements the classic accept node.
		 */
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			Matcher2.last = i;
	...
	static class LastNode extends Node {
		 ...
		 */
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			if (Matcher2.acceptMode == Matcher2.ENDANCHOR && i != Matcher2.to)
				return false;
			Matcher2.last = i;
			Matcher2.groups[0] = Matcher2.first;
			Matcher2.groups[1] = Matcher2.last;
	...
	static class Start extends Node {
		...
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			if (i > Matcher2.to - minLength) {
				Matcher2.hitEnd = true;
				return false;
			}
			boolean ret = false;
			int guard = Matcher2.to - minLength;
			for (; i <= guard; i++) {
				if (ret = next.match(Matcher2, i, seq))
					break;
				if (i == guard)
					Matcher2.hitEnd = true;
			}
	...
	static final class StartS extends Start {
		...
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			...
			int guard = Matcher2.to - minLength;
			while (i <= guard) {
				if ((ret = next.match(Matcher2, i, seq)) || i == guard)
					break;
	...
	static final class Begin extends Node {
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			int fromIndex = (Matcher2.anchoringBounds) ? Matcher2.from : 0;
			if (i == fromIndex && next.match(Matcher2, i, seq)) {
				Matcher2.first = i;
				Matcher2.groups[0] = i;
				Matcher2.groups[1] = Matcher2.last;
				return true;
	...
	static final class End extends Node {
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			int endIndex = (Matcher2.anchoringBounds) ? Matcher2.to : Matcher2
					.getTextLength();
			if (i == endIndex) {
				Matcher2.hitEnd = true;
				return next.match(Matcher2, i, seq);
			}
			return false;
		}
	}
	...
	static final class Caret extends Node {
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			int startIndex = Matcher2.from;
			int endIndex = Matcher2.to;
			if (!Matcher2.anchoringBounds) {
				startIndex = 0;
				endIndex = Matcher2.getTextLength();
			}
		...
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			if (i < Matcher2.to) {
				...
				return isSatisfiedBy(ch)
						&& next.match(Matcher2, i + Character.charCount(ch),
								seq);
			} else {
				Matcher2.hitEnd = true;
				return false;
	...
	static final class Slice extends SliceNode {
		...
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			...
			return next.match(Matcher2, i + len, seq);
		}
	}
	...
	static final class SliceS extends SliceNode {
		...
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			...
			return next.match(Matcher2, x, seq);
		}
	}
	...
	static final class Ques extends Node {
		...
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			switch (type) {
			case GREEDY:
				return (atom.match(Matcher2, i, seq) && next.match(Matcher2,
						Matcher2.last, seq)) || next.match(Matcher2, i, seq);
			case LAZY:
				return next.match(Matcher2, i, seq)
						|| (atom.match(Matcher2, i, seq) && next.match(
								Matcher2, Matcher2.last, seq));
			case POSSESSIVE:
				if (atom.match(Matcher2, i, seq))
					i = Matcher2.last;
				return next.match(Matcher2, i, seq);
			default:
				return atom.match(Matcher2, i, seq)
						&& next.match(Matcher2, Matcher2.last, seq);
			}
		}
	...
	static final class Curly extends Node {
		...
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			...
			if (type == GREEDY)
				return match0(Matcher2, i, j, seq);
			else if (type == LAZY)
				return match1(Matcher2, i, j, seq);
			else
				return match2(Matcher2, i, j, seq);
		}

		// Greedy match.
		// i is the index to start matching at
		// j is the number of atoms that have matched
		boolean match0(Matcher2 Matcher2, int i, int j, CharSequence seq) {
			if (j >= cmax) {
			...
			int backLimit = j;
			while (atom.match(Matcher2, i, seq)) {
				// k is the length of this match
				int k = Matcher2.last - i;
				...
				while (j < cmax) {
					if (!atom.match(Matcher2, i, seq))
						break;
					if (i + k != Matcher2.last) {
						if (match0(Matcher2, Matcher2.last, j + 1, seq))
							return true;
						break;
					}
					i += k;
		...
		// i is the index to start matching at
		// j is the number of atoms that have matched
		boolean match1(Matcher2 Matcher2, int i, int j, CharSequence seq) {
			for (;;) {
		...
		boolean match2(Matcher2 Matcher2, int i, int j, CharSequence seq) {
			for (; j < cmax; j++) {
				if (!atom.match(Matcher2, i, seq))
					break;
				if (i == Matcher2.last)
	...
	static final class GroupCurly extends Node {
		...
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			int[] groups = Matcher2.groups;
			int[] locals = Matcher2.locals;
			...
			for (int j = 0; j < cmin; j++) {
				if (atom.match(Matcher2, i, seq)) {
					if (capture) {
						groups[groupIndex] = i;
						groups[groupIndex + 1] = Matcher2.last;
					}
					i = Matcher2.last;
				} else {
					ret = false;
			...
			if (ret) {
				if (type == GREEDY) {
					ret = match0(Matcher2, i, cmin, seq);
				} else if (type == LAZY) {
					ret = match1(Matcher2, i, cmin, seq);
				} else {
					ret = match2(Matcher2, i, cmin, seq);
				}
			}
			if (!ret) {
		...
		boolean match1(Matcher2 Matcher2, int i, int j, CharSequence seq) {
			for (;;) {
				...
				if (i == Matcher2.last)
					return false;
				if (capture) {
					Matcher2.groups[groupIndex] = i;
					Matcher2.groups[groupIndex + 1] = Matcher2.last;
				}
				i = Matcher2.last;
				j++;
			}
	...
	static final class Branch extends Node {
		...
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			for (int n = 0; n < size; n++) {
				if (atoms[n] == null) {
					if (conn.next.match(Matcher2, i, seq))
						return true;
				} else if (atoms[n].match(Matcher2, i, seq)) {
					return true;
				}
			}
			return false;
	...
	static final class GroupHead extends Node {
		...
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			int save = Matcher2.locals[localIndex];
			Matcher2.locals[localIndex] = i;
			boolean ret = next.match(Matcher2, i, seq);
			Matcher2.locals[localIndex] = save;
			return ret;
		}

		boolean matchRef(Matcher2 Matcher2, int i, CharSequence seq) {
			int save = Matcher2.locals[localIndex];
			Matcher2.locals[localIndex] = ~i; // HACK
			boolean ret = next.match(Matcher2, i, seq);
			Matcher2.locals[localIndex] = save;
			return ret;
		}
	...
	static final class GroupRef extends Node {
		...
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			return head.matchRef(Matcher2, i, seq)
					&& next.match(Matcher2, Matcher2.last, seq);
		}
	...
	static final class GroupTail extends Node {
		...
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			int tmp = Matcher2.locals[localIndex];
			if (tmp >= 0) { // This is the normal group case.
				// Save the group so we can unset it if it
				// backs off of a match.
				int groupStart = Matcher2.groups[groupIndex];
				int groupEnd = Matcher2.groups[groupIndex + 1];

				Matcher2.groups[groupIndex] = tmp;
				Matcher2.groups[groupIndex + 1] = i;
				if (next.match(Matcher2, i, seq)) {
					return true;
				}
				Matcher2.groups[groupIndex] = groupStart;
				Matcher2.groups[groupIndex + 1] = groupEnd;
				return false;
			} else {
	...
	static final class Prolog extends Node {
		...
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			return loop.matchInit(Matcher2, i, seq);
		}
	...
	static class Loop extends Node {
		Node body;
		int countIndex; // local count index in Matcher2 locals
		...
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			...
			if (i > Matcher2.locals[beginIndex]) {
				int count = Matcher2.locals[countIndex];

				// This block is for before we reach the minimum
				// iterations required for the loop to match
				if (count < cmin) {
					Matcher2.locals[countIndex] = count + 1;
					boolean b = body.match(Matcher2, i, seq);
					// If match failed we must backtrack, so
					// the loop count should NOT be incremented
					if (!b)
						Matcher2.locals[countIndex] = count;
		...
		boolean matchInit(Matcher2 Matcher2, int i, CharSequence seq) {
			int save = Matcher2.locals[countIndex];
			boolean ret = false;
			if (0 < cmin) {
				Matcher2.locals[countIndex] = 1;
				ret = body.match(Matcher2, i, seq);
			} else if (0 < cmax) {
				Matcher2.locals[countIndex] = 1;
				ret = body.match(Matcher2, i, seq);
				if (ret == false)
					ret = next.match(Matcher2, i, seq);
			} else {
				ret = next.match(Matcher2, i, seq);
			}
			Matcher2.locals[countIndex] = save;
			return ret;
		}
	...
	static final class LazyLoop extends Loop {
		...
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			...
			if (i > Matcher2.locals[beginIndex]) {
				...
				if (count < cmin) {
					Matcher2.locals[countIndex] = count + 1;
					boolean result = body.match(Matcher2, i, seq);
					// If match failed we must backtrack, so
					// the loop count should NOT be incremented
					if (!result)
						Matcher2.locals[countIndex] = count;
		...
		boolean matchInit(Matcher2 Matcher2, int i, CharSequence seq) {
			...
			if (0 < cmin) {
				Matcher2.locals[countIndex] = 1;
				ret = body.match(Matcher2, i, seq);
			} else if (next.match(Matcher2, i, seq)) {
				ret = true;
			} else if (0 < cmax) {
				Matcher2.locals[countIndex] = 1;
				ret = body.match(Matcher2, i, seq);
	...
	static class BackRef extends Node {
		...
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			int j = Matcher2.groups[groupIndex];
			int k = Matcher2.groups[groupIndex + 1];
			...
			return next.match(Matcher2, i + groupSize, seq);
		}
	...
	static final class First extends Node {
		...
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			...
			for (;;) {
				...
				if (atom.match(Matcher2, i, seq)) {
					return next.match(Matcher2, Matcher2.last, seq);
				}
	...
	static final class Conditional extends Node {
		...
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			if (cond.match(Matcher2, i, seq)) {
				return yes.match(Matcher2, i, seq);
			} else {
				return not.match(Matcher2, i, seq);
			}
		}
	...
	static final class Pos extends Node {
		...
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			int savedTo = Matcher2.to;
			boolean conditionMatched = false;

			// Relax transparent region boundaries for lookahead
			if (Matcher2.transparentBounds)
				Matcher2.to = Matcher2.getTextLength();
			try {
				conditionMatched = cond.match(Matcher2, i, seq);
			} finally {
				// Reinstate region boundaries
				Matcher2.to = savedTo;
			}
			return conditionMatched && next.match(Matcher2, i, seq);
		}
	}
	...
	static Node lookbehindEnd = new Node() {
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			return i == Matcher2.lookbehindTo;
		}
	};
	...
	static class Behind extends Node {
		...
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			int savedFrom = Matcher2.from;
			boolean conditionMatched = false;
			int startIndex = (!Matcher2.transparentBounds) ? Matcher2.from : 0;
			int from = Math.max(i - rmax, startIndex);
			// Set end boundary
			int savedLBT = Matcher2.lookbehindTo;
			Matcher2.lookbehindTo = i;
			// Relax transparent region boundaries for lookbehind
			if (Matcher2.transparentBounds)
				Matcher2.from = 0;
			for (int j = i - rmin; !conditionMatched && j >= from; j--) {
				conditionMatched = cond.match(Matcher2, j, seq);
			}
			Matcher2.from = savedFrom;
			Matcher2.lookbehindTo = savedLBT;
			return conditionMatched && next.match(Matcher2, i, seq);
		}
	}
	...
	static final class Bound extends Node {
		...
		int check(Matcher2 Matcher2, int i, CharSequence seq) {
			int ch;
			boolean left = false;
			int startIndex = Matcher2.from;
			int endIndex = Matcher2.to;
		...
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			return (check(Matcher2, i, seq) & type) > 0
					&& next.match(Matcher2, i, seq);
		}
	}
	 ...
	 */
	private static boolean hasBaseCharacter(Matcher2 Matcher2, int i,
			CharSequence seq) {
		int start = (!Matcher2.transparentBounds) ? Matcher2.from : 0;
	...
	static class BnM extends Node {
		...
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			int[] src = buffer;
			int patternLength = src.length;
			int last = Matcher2.to - patternLength;
			...
			NEXT: while (i <= last) {
				...
				Matcher2.first = i;
				boolean ret = next.match(Matcher2, i + patternLength, seq);
				if (ret) {
					Matcher2.first = i;
					Matcher2.groups[0] = Matcher2.first;
					Matcher2.groups[1] = Matcher2.last;
	...
	static final class BnMS extends BnM {
		...
		boolean match(Matcher2 Matcher2, int i, CharSequence seq) {
			int[] src = buffer;
			int patternLength = src.length;
			int last = Matcher2.to - lengthInChars;
			...
			NEXT: while (i <= last) {
				...
				// Entire pattern matched starting at i
				Matcher2.first = i;
				boolean ret = next.match(Matcher2, i + lengthInChars, seq);
				if (ret) {
					Matcher2.first = i;
					Matcher2.groups[0] = Matcher2.first;
					Matcher2.groups[1] = Matcher2.last;
