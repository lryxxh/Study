 ...
 * @since 1.0.6
 */
public class CustomRendererUtilities {
    ...
    public static int findLiveItemsUpperBound(XYDataset dataset, int series,
        ...
        if (dataset.getDomainOrder() == DomainOrder.ASCENDING) {
            ...
            int high = itemCount - 1;
            int mid = (low + high) / 2;
            double lowValue = dataset.getXValue(0, low);
            if (lowValue < xLow) {
                return low;
            }
            double highValue = dataset.getXValue(0, high);
            if (highValue >= xLow) {
                return high;
            }
            while (high - low > 1) {
                double midV = dataset.getXValue(0, mid);
                if (midV >= xLow) {
                    low = mid;
                }
                else {
