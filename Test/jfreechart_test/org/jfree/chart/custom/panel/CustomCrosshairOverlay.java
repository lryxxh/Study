...
package org.jfree.chart.custom.panel;
 ...
 * @since 1.0.13
 */
public class CustomCrosshairOverlay extends AbstractOverlay implements Overlay,
...
//                } 
                if (plot.getOrientation() == PlotOrientation.VERTICAL) {
                    drawVerticalCrosshair(g2, dataArea, x, ch, chartPanel);
                }
                else {
                    drawHorizontalCrosshair(g2, dataArea, x, ch);
                }
            }
        }
        ...
        while (iterator.hasNext()) {
            ...
            if (ch.isVisible()) {
                ...
                if (plot.getOrientation() == PlotOrientation.VERTICAL) {
                    ...
                    drawVerticalCrosshair(g2, dataArea, yy, ch, chartPanel);
                }
            }
        }
    ...
    protected void drawVerticalCrosshair(Graphics2D g2, Rectangle2D dataArea,
            double x, Crosshair crosshair,ChartPanel chartPanel) {
    	if (!chartPanel.getChart().getXYPlot().isDomainCrosshairVisible()) {
    		return;
    	}
    	if (x >= dataArea.getMinX() && x <= dataArea.getMaxX()) {
    		x = (int) (x * chartPanel.getScaleX() + chartPanel.getInsets().left);
            ...
            if (crosshair.isLabelVisible()) {
                String label = crosshair.getLabelGenerator().generateLabel(
                        crosshair);
                double domainValuePoint = crosshair.getValue();
                JFreeChart chart = chartPanel.getChart();
                XYPlot plot = (XYPlot) chart.getPlot();
                ValueAxis domainAxis = plot.getDomainAxis();
                NumberAxis rangeAxis = (NumberAxis) plot.getRangeAxis();
                Rectangle2D rectangle2D = chartPanel.getChartRenderingInfo().getPlotInfo()
    			.getDataArea();
                RectangleEdge domainRectangleEdge = plot.getDomainAxisEdge();
                String[] fileText = new String[]{};
                List<Object> crosshairList = new ArrayList<Object>();
                int datasetCount = plot.getDatasetCount();
                for(int i = 0; i < datasetCount; i++) {
                	XYDataset dataset = plot.getDataset(i);
                	int seriesCount = dataset.getSeriesCount();
                	for (int j = 0; j < seriesCount; j++) {
                		double domainValue = domainAxis.java2DToValue(domainValuePoint, rectangle2D, domainRectangleEdge);
                		if(domainAxis instanceof CustomDateAxis) {
                			domainValue = ((CustomDateAxis)domainAxis).java2DToValueCustom(domainValuePoint, rectangle2D, domainRectangleEdge, j);
                			format = ((CustomDateAxis)domainAxis).getDateFormatOverride();
                		}
                		if (chartPanel instanceof CustomChartPanel) {
                			int crossIndex = ((CustomChartPanel) chartPanel).getIndexByXValue(domainValue,i, j);
                			if (crossIndex != -1) {
                				domainValue = dataset.getXValue(j, crossIndex);
                			}
                		}
                		numberformat = rangeAxis.getNumberFormatOverride();
                		String crosshairString = "";
                		Paint color = Color.RED;
                		color =  plot.getRenderer(i).getSeriesPaint(j);
                		if (dataset instanceof TimeSeriesCollection) {
                			TimeSeries timeSeries = ((TimeSeriesCollection) dataset).getSeries(j);
                			RegularTimePeriod timePeriod = RegularTimePeriod.createInstance(timeSeries.getTimePeriodClass(), new Date((long)domainValue), TimeZone.getDefault());
                			Number value = timeSeries.getValue(timePeriod);
                			if (value != null) {
                				crosshairString += format.format(new Date((long)domainValue)) + " " + numberformat.format(value.doubleValue());
                			} else {
                				crosshairString += format.format(new Date((long)domainValue)) +" null";
                			}
                		} else if(dataset instanceof XYSeriesCollection){
                			XYSeries xySeries = ((XYSeriesCollection)dataset).getSeries(j);
                			int index = xySeries.indexOf(domainValue);
                			if (index >= 0) {
                				crosshairString += numberformat.format(domainValue) + " " + xySeries.getY(index);
                			} else {
                				crosshairString += numberformat.format(domainValue) + " null";
                			}
                		}
                		crosshairList.add(crosshairString);
                		crosshairList.add(color);
                	}
                }
                fileText = new String[crosshairList.size() / 2];
                Color colors[] = new Color[crosshairList.size() / 2];
                for (int i=0;i<crosshairList.size() / 2;i++) {
             	   fileText[i] = (String) crosshairList.get(i * 2);
             	   colors[i] = (Color)crosshairList.get(i * 2 + 1);
                }
            
                RectangleAnchor anchor = crosshair.getLabelAnchor();
                Point2D pt = calculateLabelPoint(line, anchor, 5, 5);
                float xx = (float) pt.getX() - 5;
                float yy = (float) pt.getY() - 5;
                TextAnchor alignPt = textAlignPtForLabelAnchorV(anchor);
                Shape hotspot = calculateRotatedStringBounds(
                		fileText, g2, xx, yy, anchor, 0.0, TextAnchor.CENTER, dataArea);
                if (!dataArea.contains(hotspot.getBounds2D())) {
                    ...
                    hotspot = calculateRotatedStringBounds(
                    		fileText, g2, xx, yy, anchor, 0.0, TextAnchor.CENTER,dataArea);
                }
                g2.setPaint(crosshair.getLabelBackgroundPaint());
                g2.fill(hotspot);
                ...
                g2.draw(hotspot);
//                if (domainAxis instanceof CustomDateAxis) {
                	xx = (float) hotspot.getBounds().getX();
                	yy = (float) hotspot.getBounds().getY();
             		for (int i = 0; i < fileText.length; i++) {
             			AttributedString as = new AttributedString(fileText[i], g2
             					.getFont().getAttributes());

             			LineBreakMeasurer measurer = new LineBreakMeasurer(as.getIterator(), g2
             					.getFontRenderContext());
             			int wrappingWidth = (int) (dataArea.getWidth() - 15);
             			while (measurer.getPosition() < fileText[i].length()) {
             				TextLayout layout = measurer.nextLayout(wrappingWidth);
             				yy += (layout.getAscent()) + 5;
             				float dx = layout.isLeftToRight() ? 5 : (wrappingWidth - layout
             						.getAdvance());
             				g2.setColor(colors[i]);
             				layout.draw(g2, xx + dx, yy);
...
//                }
    ...
    private Shape calculateRotatedStringBounds(String[] labels, Graphics2D g2,
			float xx, float yy, RectangleAnchor anchor, double d, TextAnchor center, Rectangle2D dataArea) {
    	...
    	if (anchor.equals(RectangleAnchor.BOTTOM_LEFT) || anchor.equals(RectangleAnchor.TOP_LEFT) || anchor.equals(RectangleAnchor.LEFT)) {
    		if ((xx - width - 5) < dataArea.getX()) {
    			xx = xx + 10;
    		} else {
    			xx = xx - width - 5;
    		}
    		...
    		if ((xx + width + 10) > (dataArea.getWidth() + dataArea.getX())) {
        		xx = xx - width - 15;
        	}
    	}
    	if (yy + width + 5 > dataArea.getHeight()) {
    		yy = yy - height - 15;
    	}
    	shape = new Rectangle2D.Double(xx + 5, yy + 5, width + 5, height);
    ...
    public boolean equals(Object obj) {
        ...
        CustomCrosshairOverlay that = (CustomCrosshairOverlay) obj;
    ...
    public Object clone() throws CloneNotSupportedException {
    	CustomCrosshairOverlay clone = (CustomCrosshairOverlay) super.clone();
