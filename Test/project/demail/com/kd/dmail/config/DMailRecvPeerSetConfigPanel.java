 ...
 * @author xuzhiqi
 */
public class DMailRecvPeerSetConfigPanel extends PaneImpl {

	private static final long serialVersionUID = 1L;

	public DMailRecvPeerSetConfigPanel() {
		initComponents();
		setPreferredSize(new java.awt.Dimension(500, 400));
		jbInit();
	}

	String flag = "[DMailRecvPeerSet]";
	...
	public void jbInit() {
		...
		try {
			...
			jTextField2.setText(map.get("Port"));
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			try {
	...
	public void saveAndUpload() throws Exception {
		if (check()) {
			...
			sb.append("Port=" + jTextField2.getText().toString().trim() + "\n");
			InputStream input = null;
			InputStreamReader reader = null;
			BufferedReader br = null;
			String result = "";
			...
			new SFTPTool().uploadFile(
					...
					Configuration.config.getProperty("password"),
					Configuration.config.getProperty("hostpath"), "dmail.cfg");
			JOptionPane.showMessageDialog(DMailRecvPeerSetConfigPanel.this,
					"文件已保存并上传至服务器");
		}
	}
